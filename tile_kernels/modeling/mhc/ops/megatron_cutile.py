...
"""Megatron-compatible fused cuTile kernels for mHC.

Requires cuda.tile (cuTile) for optimal performance on supported GPUs
(compute capability 10.x+).  This TileKernels copy is self-contained and keeps
the public Megatron kernel call signatures for benchmarking.
...
def is_triton_available() -> bool:
    """Return True if Triton hpb backward kernels are available."""
...
if _CUTILE_AVAILABLE:
    ...
    # -- Fused 3-way elementwise add -----------------------------------------
    # NOTE: We intentionally do NOT use a cuTile kernel for this operation.
    # NCU profiling showed cuTile generates 56 registers/thread for a trivial
    # a+b+c elementwise, capping occupancy at 56% and wasting register file.
    # torch.compile produces a far more efficient pointwise kernel (~12 regs,
    # near-100% occupancy) for the same op.  fused_add_3 is defined above.
    pass
...
if not _CUTILE_AVAILABLE:
    ...
    fused_proj_rms_compute_h = _no_cutile_error
    ...
    class FusedProjRmsComputeH(torch.autograd.Function):
        """Fused projection + RMS norm + compute_h activations (cuTile).

        Forward: _ct_proj_rms_fwd_kernel -> _ct_reduce_compute_h_kernel
        Backward: _ct_fused_compute_h_proj_rms_bwd_kernel (single fused kernel)
        """
        ...
        def forward(
            ...
            """cuTile fused proj_rms + compute_h forward."""
        ...
        def backward(ctx, grad_y, grad_r_ext):
            """Fused backward: single kernel for compute_h + proj_rms."""
            (
    ...
    def fused_proj_rms_compute_h(
        ...
        """Fused projection + RMS norm + compute_h activations.

        Args:
            x: [M, K] input
            weight: [N, K] projection weight
            alpha_pre: scalar Tensor (learnable gating for h_pre)
            alpha_post: scalar Tensor (learnable gating for h_post)
            alpha_res: scalar Tensor (learnable gating for h_res)
            bias: [N] bias parameter
            n: number of residual streams
            eps: stability epsilon

        Returns:
            y_activated: [M, N] with h_pre(sigmoid) | h_post(2*sigmoid) | h_res(identity)
            r: [M, 1] = norm / sqrt(K)
        """
        return FusedProjRmsComputeH.apply(
            x, weight, alpha_pre, alpha_post, alpha_res, bias, n, eps
        )

# ============================================================================
# Transformer Engine (TE) Triton kernel wrappers
# ============================================================================
# These wrappers present the same public API as the cuTile fused functions
# above, but delegate to the Triton-based kernels shipped in
# ``transformer_engine.pytorch.triton.mhc``.
# ============================================================================

_TE_MHC_AVAILABLE = False
try:
    from transformer_engine.pytorch.triton.mhc import (
        mhc_fused_sinkhorn as _te_sinkhorn,
        mhc_fused_aggregate as _te_aggregate,
        mhc_fused_expand_combine as _te_expand_combine,
        mhc_fused_projection as _te_projection,
    )

    _TE_MHC_AVAILABLE = True
except ImportError:
    pass


def is_te_mhc_available() -> bool:
    """Return True if TE Triton fused MHC kernels are available."""
    return _TE_MHC_AVAILABLE


if not _TE_MHC_AVAILABLE:

    def _no_te_mhc_error(*_args, **_kwargs):
        raise RuntimeError(
            "TE Triton fused mHC kernels require transformer_engine with triton MHC "
            "support. Either install the required transformer_engine version or set "
            "mhc_use_te_kernels=False."
        )

    te_fused_sinkhorn = _no_te_mhc_error
    te_fused_h_aggregate = _no_te_mhc_error
    te_fused_h_post_bda = _no_te_mhc_error
    te_fused_proj_rms = _no_te_mhc_error

else:

    def te_fused_sinkhorn(input_logits: Tensor, num_iterations: int, eps: float = 1e-6) -> Tensor:
        """TE Triton Sinkhorn-Knopp projection (same API as cuTile ``fused_sinkhorn``)."""
        n = input_logits.shape[-1]
        return _te_sinkhorn(input_logits, n, iters=num_iterations)

    def te_fused_h_aggregate(x: Tensor, h_pre: Tensor) -> Tensor:
        """TE Triton weighted aggregation (same API as cuTile ``fused_h_aggregate``).

        The cuTile API expects ``x`` in [s, b, n, C] layout, while TE Triton
        expects [s, b, C, n].  We use ``.transpose()`` (metadata-only, zero-copy)
        without ``.contiguous()`` — the TE Triton kernel handles strided inputs
        natively.
        """
        n = x.shape[-2]
        # [s, b, n, C] -> [s, b, C, n] (metadata-only, no data copy)
        x_t = x.transpose(-1, -2)
        return _te_aggregate(x_t, h_pre, n)

    def te_fused_h_post_bda(
        h_res: Tensor, original_residual: Tensor, h_post: Tensor, x: Tensor, bias: Optional[Tensor]
    ) -> Tensor:
        """TE Triton fused expand-combine (same API as cuTile ``fused_h_post_bda``).

        Translates from cuTile arg order ``(h_res, original_residual, h_post, x, bias)``
        to TE Triton ``mhc_fused_expand_combine(x, bias, h_post, original_residual, h_res, n, True)``.

        cuTile expects ``original_residual`` in [s, b, n, C] while TE expects [s, b, C, n].
        Input: ``.transpose()`` only (metadata, zero-copy) — TE Triton handles strided inputs.
        Output: ``.transpose().contiguous()`` required because callers use ``.view()``
        on the result which needs a contiguous tensor.
        """
        n = h_res.shape[-1]
        # [s, b, n, C] -> [s, b, C, n] (metadata-only, no data copy)
        orig_t = original_residual.transpose(-1, -2)
        out = _te_expand_combine(x, bias, h_post, orig_t, h_res, n, True)
        # [s, b, C, n] -> [s, b, n, C] (must be contiguous for caller's .view())
        return out.transpose(-1, -2).contiguous()

    def te_fused_proj_rms(x: Tensor, weight: Tensor, eps: float = 1e-6) -> Tuple[Tensor, Tensor]:
        """TE Triton fused projection + RMS (same API as cuTile ``fused_proj_rms``).

        TE projection may return more columns than ``weight.shape[0]``; we
        truncate to the first ``N`` columns to match the cuTile output layout.
        """
        proj, r = _te_projection(x, weight)
        N = weight.shape[0]
        if proj.shape[-1] != N:
            proj = proj[..., :N]
        return proj, r
